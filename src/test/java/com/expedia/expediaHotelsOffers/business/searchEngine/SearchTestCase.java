...
public class SearchTestCase extends TestCase {
	...
	public void testDoSearch() {
		...
		assertTrue(hotel1.size()==1);
		
		filter.put("destinationCountry", new String[]{"United States of America"});
		List <Hotel> hotel2 = search.doSearch(filter);
		assertNotNull(hotel2);
		assertTrue(hotel2.size()==1);
		
		filter.put("destinationCountry", new String[]{"Jordan"});
		List <Hotel> hotel3 = search.doSearch(filter);
		assertNotNull(hotel3);
		assertTrue(hotel3.size()==0);
		
		
		
	}

	ExpediaJson createExpediaJSON() {
		...
		ExpediaJson expediaJson = new ExpediaJson();
